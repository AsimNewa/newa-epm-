...
import type { ImportFormat } from '../lib/member-format';
import { FORMAT_LABELS, detectFormat, parseImportFile, unflattenToMemberDto } from '../lib/member-format';
...
const STANDARD_MEMBER_FIELDS = ['dimension', 'code', 'name', 'parentCode', 'weight', 'status'];

interface DimensionMemberImportModalProps {
  open: boolean;
  onClose: () => void;
  /** Used to fill in the "dimension" column for rows that omit it — the dimension currently being viewed. */
  defaultDimension: string;
  onImport: (rows: Record<string, unknown>[]) => Promise<ImportResult>;
}

export function DimensionMemberImportModal({
  open,
  onClose,
  defaultDimension,
  onImport,
}: DimensionMemberImportModalProps): JSX.Element | null {
  ...
  const handleImport = async (): Promise<void> => {
    ...
    setIsImporting(true);
    setResult(null);
    // Rows that omit the "dimension" column fall back to the dimension currently being viewed.
    const dtos = parsedRows.map((row) => {
      const dto = unflattenToMemberDto(row);
      if (!dto.dimension) dto.dimension = defaultDimension;
      return dto;
    });
    try {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="flex w-full max-w-3xl flex-col gap-4 rounded bg-white p-6 shadow-xl">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-slate-800">Import Dimension Members</h2>
          <button className="text-slate-400 hover:text-slate-700" onClick={handleClose}>✕</button>
        </div>

        <div className="rounded border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">
          <strong>Standard columns:</strong> {STANDARD_MEMBER_FIELDS.join(', ')}
          <br />
          <span className="text-slate-500">
            <code>dimension</code> selects which dimension the row belongs to (matches a dimension's{' '}
            <strong>name</strong> or <strong>type</strong>, e.g. &ldquo;Cost Center&rdquo; or
            &ldquo;COST_CENTER&rdquo;) — a single file can mix rows for multiple dimensions. Rows that leave{' '}
            <code>dimension</code> blank default to <strong>{defaultDimension}</strong>, the dimension you're
            currently viewing.
            <br />
            <code>parentCode</code> must reference a member <strong>already created in that same dimension</strong>.
            Row order matters — list parent rows before their children (a blank <code>parentCode</code> is a
            root member).
          </span>
        </div>
        ...
        {result && (
          ...
          >
            <strong>✅ {result.created} member{result.created !== 1 ? 's' : ''} imported</strong>
