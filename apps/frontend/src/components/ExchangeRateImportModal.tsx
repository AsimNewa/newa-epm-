...
import { FORMAT_LABELS, detectFormat, parseImportFile, unflattenToExchangeRateDto } from '../lib/currency-format';
...
export function ExchangeRateImportModal({ open, onClose, onImport }: Props): JSX.Element | null {
  ...
  const handleImport = async () => {
    ...
    try {
      const r = await onImport(parsedRows.map(unflattenToExchangeRateDto));
      setResult(r);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="flex w-full max-w-2xl flex-col gap-4 rounded bg-white p-6 shadow-xl">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-slate-800">Import Exchange Rates</h2>
          <button className="text-slate-400 hover:text-slate-700" onClick={handleClose}>✕</button>
        </div>

        <div className="rounded border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">
          <strong>Columns:</strong> fromCurrency, toCurrency, rateDate (YYYY-MM-DD), rate (decimal), rateType
          <br />
          <span className="text-slate-500">Existing rates for the same pair + date + rateType will be updated (upsert).</span>
        </div>
        ...
        {result && (
          <div className={`rounded p-3 text-sm ${result.errors.length === 0 ? 'bg-green-50 text-green-800' : 'bg-amber-50 text-amber-800'}`}>
            <strong>✅ {result.created} rates imported</strong>
