...
import type { ImportFormat } from '../lib/generic-format';
import { FORMAT_LABELS, detectFormat, parseImportFile } from '../lib/generic-format';
...
interface GenericImportModalProps {
  ...
  helpText?: ReactNode;
  /** Converts one parsed file row into the create/upsert DTO shape expected by the import endpoint. */
...
export function GenericImportModal({
  ...
  fromRow,
  onImport,
}: GenericImportModalProps): JSX.Element | null {
  ...
  const handleImport = async (): Promise<void> => {
    ...
    setIsImporting(true);
    setResult(null);
    const dtos = parsedRows.map((row) => fromRow(row));
    try {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="flex w-full max-w-3xl flex-col gap-4 rounded bg-white p-6 shadow-xl">
        ...
        <div className="rounded border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">
          ...
          {helpText && <div className="mt-1 text-slate-500">{helpText}</div>}
        </div>
        ...
        {result && (
          ...
          >
            <strong>✅ {result.created} record{result.created !== 1 ? 's' : ''} imported</strong>
