...
import type { ImportFormat } from '../lib/dimension-format';
import {
  FORMAT_LABELS,
  detectFormat,
  parseImportFile,
  unflattenToDimensionDto,
} from '../lib/dimension-format';
...
const STANDARD_FIELDS = ['name', 'type', 'status'];
...
export function DimensionImportModal({
  open,
  onClose,
  customFieldDefs,
  onImport,
}: DimensionImportModalProps): JSX.Element | null {
  ...
  const handleImport = async (): Promise<void> => {
    ...
    setIsImporting(true);
    setResult(null);
    const dtos = parsedRows.map((row) => unflattenToDimensionDto(row, customFieldDefs));
    try {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="flex w-full max-w-3xl flex-col gap-4 rounded bg-white p-6 shadow-xl">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-slate-800">Import Dimensions</h2>
          <button className="text-slate-400 hover:text-slate-700" onClick={handleClose}>✕</button>
        </div>

        <div className="rounded border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">
          <strong>Standard columns:</strong> {STANDARD_FIELDS.join(', ')}
          {cfFields.length > 0 && (
            <>
              <br />
          ...
          <span className="text-slate-500">
            A dimension with an existing <code>name</code> is updated in place; otherwise a new dimension is created.
          </span>
        </div>
        ...
        {result && (
          ...
          >
            <strong>✅ {result.created} dimension{result.created !== 1 ? 's' : ''} imported</strong>
