...
import type { ImportFormat } from '../lib/entity-format';
import {
  FORMAT_LABELS,
  detectFormat,
  parseImportFile,
  unflattenToEntityDto,
...
export function EntityImportModal({
  ...
  customFieldDefs,
  onImport,
}: EntityImportModalProps): JSX.Element | null {
  ...
  const handleImport = async (): Promise<void> => {
    ...
    // Convert parsed flat rows into entity DTOs (handle customFields prefix)
    const dtos = parsedRows.map((row) => unflattenToEntityDto(row, customFieldDefs));
  ...
  const standardFields = ['code', 'name', 'currency', 'country', 'status'];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="flex w-full max-w-3xl flex-col gap-4 rounded bg-white p-6 shadow-xl">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-slate-800">Import Entities</h2>
        ...
        <div className="rounded border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">
          <strong>Standard columns:</strong> {standardFields.join(', ')}
          {cfFields.length > 0 && (
            <>
              <br />
          ...
          <span className="text-slate-500">
            Custom fields can be provided as <code>customFields.key</code> or just <code>key</code> if
            the name matches a defined custom field.
          </span>
        </div>

        {/* Format + file controls */}
        ...
        {preview.length > 0 && (
          <div className="overflow-x-auto">
            ...
            <p className="mt-1 text-xs text-slate-400">
              Orange columns = custom fields. Standard columns are grey.
            </p>
          </div>
        )}
