...
import { FORMAT_LABELS, detectFormat, parseImportFile, unflattenToGroupDto } from '../lib/ownership-format';
...
const STANDARD_FIELDS = ['code', 'name', 'reportingCurrency', 'status'];
...
export function GroupImportModal({
  ...
  customFieldDefs,
  onImport,
}: GroupImportModalProps): JSX.Element | null {
  ...
  const handleImport = async (): Promise<void> => {
    ...
    setIsImporting(true);
    setResult(null);
    const dtos = parsedRows.map((row) => unflattenToGroupDto(row, customFieldDefs));
    try {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="flex w-full max-w-3xl flex-col gap-4 rounded bg-white p-6 shadow-xl">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-slate-800">Import Consolidation Groups</h2>
        ...
        {result && (
          ...
          >
            <strong>✅ {result.created} group{result.created !== 1 ? 's' : ''} imported</strong>
