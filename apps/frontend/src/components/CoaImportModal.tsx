...
import type { CustomFieldDefinition } from '@newa-epm/shared';
import type { ImportFormat } from '../lib/coa-format';
import { FORMAT_LABELS, detectFormat, parseImportFile, unflattenToCoaDto } from '../lib/coa-format';
...
const STANDARD_COA_FIELDS = [
  'accountCode', 'accountName', 'accountType', 'accountNature',
  'parentCode', 'rollupWeight', 'status', 'statementType',
  'cashFlowCategory', 'ifrsReference', 'requiresIntercompanyRecon', 'requiresOtherRecon',
];
...
export function CoaImportModal({
  open,
  onClose,
  customFieldDefs,
  onImport,
}: CoaImportModalProps): JSX.Element | null {
  ...
  const handleImport = async (): Promise<void> => {
    ...
    setIsImporting(true);
    setResult(null);
    const dtos = parsedRows.map((row) => unflattenToCoaDto(row, customFieldDefs));
    try {
  ...
  const cfFields = customFieldDefs.map((d) => `customFields.${d.fieldKey}`);

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="flex w-full max-w-3xl flex-col gap-4 rounded bg-white p-6 shadow-xl">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-slate-800">Import Chart of Accounts</h2>
          <button className="text-slate-400 hover:text-slate-700" onClick={handleClose}>✕</button>
        </div>

        <div className="rounded border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">
          <strong>Standard columns:</strong> {STANDARD_COA_FIELDS.join(', ')}
          {cfFields.length > 0 && (
            <>
              <br />
              <strong>Custom-field columns:</strong> {cfFields.join(', ')}
            </>
          )}
          <br />
          <span className="text-slate-500">
            Boolean fields (<code>requiresIntercompanyRecon</code>, <code>requiresOtherRecon</code>)
            accept <code>true</code>/<code>false</code> or <code>yes</code>/<code>no</code>.
            Blank <code>parentCode</code> is treated as a root account.
          </span>
        </div>
        ...
        {result && (
          ...
          >
            <strong>✅ {result.created} account{result.created !== 1 ? 's' : ''} imported</strong>
